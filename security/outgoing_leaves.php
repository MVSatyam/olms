...
if ($_SESSION['security'] != NULL) {
    ...
    $sql = "SELECT * FROM leaves WHERE permission='granted' and outgoing='no' ORDER BY id ASC";
    ...
    if ($count == 0) {
        ...
        while ($leave_row = mysqli_fetch_assoc($query)) {
            ...
            $all_outgoing_leaves = $all_outgoing_leaves . '<tr>
                                            ...
                                            <td><button class="btn btn-outline-danger btn-sm waves-effect p-1" id="markAsLeaveGone" style="" data-userid="' . $leave_row['user_id'] . '" data-leaveid="' . $leave_row['id'] . '">Mark</button></td>
                                        </tr>';
        }
    ...
    <body class="skin-light">
        ...
        <main>
            <div class="container" style="margin-top: 100px;">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title text-center"><a>All Outgoing Leaves</a></h4>
                        <div id="all_outgoing_leaves">
                            <?php echo $all_outgoing_leaves; ?>
                        </div>
                    </div>
                </div>
        ...
        <script type="text/javascript">
            ...
            $(document).on('click', '#markAsLeaveGone', function() {
                var user_id = $(this).data('userid');
                var leave_id = $(this).data('leaveid');

                $.ajax({
                    url: 'mark_as_leave_gone.php',
                    type: 'post',
                    data: 'user_id=' + user_id + '&leave_id=' + leave_id,
                    success: function(result) {
                        // console.log(result);
                        $('#all_outgoing_leaves').html(result);
                    }
                })
            })
        </script>
